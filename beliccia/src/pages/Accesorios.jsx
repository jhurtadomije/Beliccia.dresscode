...
function isAccesorio(p) {
  ...
  return (
    terms.some((t) => ACCESSORY_WORDS.has(t)) ||
    inferGroupFromTerms(terms) !== null
  );
}

// ⚠️ Priorizar slug, luego id...
...
function AccesorioCard({ producto, onSolicitarInfo }) {
  ...
  const showPrice = price !== null && price <= PRICE_LIMIT; // si > 250 no mostramos el precio
            ...
            {canBuy ? (
              <button
                ...
                onClick={() =>
                  addToCart({
                    ...producto,
                    precio: price ?? 0, // adaptamos al carrito
                  })
                }
              >
                Añadir al carrito
...
export default function Accesorios() {
  ...
  const filtroCat = ['tocados', 'bolsos', 'pendientes'].includes(
    segmento
  )
    ? segmento
    : null;
  ...
  useEffect(() => {
    ...
    api
      .get('/productos', {
        params: { categoria: 'complementos', limite: 1000 },
      })
      .then(({ data }) => {
        ...
        if (filtroCat) {
          accesorios = accesorios.filter(
            (p) =>
              inferGroupFromTerms(productTerms(p)) === filtroCat
          );
        }
    ...
    >
      <div className="container">
        <h2 className="text-center mb-4">
          {filtroCat
            ? `Accesorios: ${
                filtroCat.charAt(0).toUpperCase() +
                filtroCat.slice(1)
              }`
